...
public class DatastoreCacheProxyHook<E extends Environment> implements Delegate<E> {
  ...
  private static final String[] IGNORE_KINDS = {CacheService.RESET_DATE_KIND, "_ah_SESSION"};
  ...
  public Future<byte[]> makeAsyncCall(
      E environment, String packageName, String methodName, byte[] request, ApiConfig apiConfig) {
  ...
  public byte[] makeSyncCall(E environment, String packageName, String methodName, byte[] request)
   ...
   * @return
   */
  private byte[] delegateRunQuery(
      E environment, String packageName, String methodName, byte[] request) {
  ...
  static class SyncFuture implements Future<byte[]> {
    ...
    public byte[] get(long l, TimeUnit timeunit)
